...
USAGE="Usage: $0 [init|measure|analyze] [folder]"
...
if [ "$1" = "measure" ]; then
    ...
    do
	    ...
	    ~/Development/tcp-eval/measurement/tcp-ancr_eval/$measurement.py pair.conf -l $FOLDER/${measurement} 2>&1 | tee -a $LOG
        ssh puneeth@192.168.5.1 "cd /tmp && tar -czvf ${measurement}.tar.gz *.pcap && cp ${measurement}.tar.gz ~/"
    done
fi
